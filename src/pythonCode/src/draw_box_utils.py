...
def draw_objs(image: Image,
    ...
    if mouse_data:
        ...
        if len(mouse_data["ear_poses"]) >= 2:
            draw.line([*mouse_data["ear_poses"][0], *mouse_data["ear_poses"][1]], width=line_thickness, fill=data_color)

        # drawing direction from head direction
        ray_dist = 30
        # drawing direction from angle
        draw.line([*mouse_data["ears_center"], (mouse_data["ears_center"][0] + math.cos(mouse_data["head_angle"]) * ray_dist, mouse_data["ears_center"][1] + math.sin(mouse_data["head_angle"]) * ray_dist)], width=line_thickness, fill="green")

        if len(mouse_data["tail_pos"]) == 2:
            draw.ellipse([(mouse_data["tail_pos"][0]-10, mouse_data["tail_pos"][1]-10), (mouse_data["tail_pos"][0]+10, mouse_data["tail_pos"][1]+10)], width=3, outline="blue")
            draw.line([(image.size[0]*0.5, image.size[1]*0.5), *mouse_data["tail_pos"]], width=line_thickness, fill="blue")

        if mouse_data["head_angle"] is not None and len(mouse_data["tail_pos"]) != 0:
            # drawing text
            font = ImageFont.truetype("arial.ttf", 18)
            draw.text((10, 40),
                      f"head angle: {math.floor(mouse_data['head_angle'] * 180 / math.pi * 100000) / 100000} | tail angle: {math.floor(mouse_data['tail_angle'] * 180 / math.pi * 1000) / 1000}",
                      fill="black",
                      font=font)
